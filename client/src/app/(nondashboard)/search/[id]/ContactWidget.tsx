...
import { useGetAuthUserQuery, useGetManagerDetailsQuery } from "@/state/api";
import { Phone } from "lucide-react";
import { useRouter } from "next/navigation";
import React from "react";

const ContactWidget = ({ managerId, onOpenModal }: ContactWidgetProps) => {
  const { data: authUser } = useGetAuthUserQuery();
  const { data: manager } = useGetManagerDetailsQuery(managerId ?? "", {
    skip: !managerId,
  });
      ...
      <div className="flex items-center gap-5 mb-4 border border-primary-200 p-4 rounded-xl">
        ...
        <div>
          ...
          <div className="text-lg font-bold text-primary-800">
            {manager?.phoneNumber ?? "(444) 222-3333"}
          </div>
        </div>
      </div>
      <Button
